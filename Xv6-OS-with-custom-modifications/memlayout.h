...
#define SHAREDTOP 0xE000000         // Top shared memory
#define SHAREDSIZE (1 << 12)        // size shared memory
#define PHYSTOP (SHAREDTOP - SHAREDSIZE) // Top physical memory
...
static inline uint32_t V2P(void *a) {
    ...
    return (uint32_t)a - KERNBASE;
}

// Convert physical address to kernel virtual address
static inline void *P2V(uint32_t a) {
    extern void panic(char*) __attribute__((noreturn));
    if (a > KERNBASE)
...
#define V2P_C(x) (((uint32_t) x) - KERNBASE)
