...
// Circular queue from https://www.geeksforgeeks.org/c-program-to-implement-circular-queue/
...
int
upperread(struct inode *ip, char *dst, int n)
{
    iunlock(ip);
...
int
upperwrite(struct inode *ip, char *buf, int n)
{
    iunlock(ip);
